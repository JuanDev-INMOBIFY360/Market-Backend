...
export class ProductController {
    ...
    create = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            const {
                barcode, name, salePrice, purchasePrice,
                categoryId, description, offerPrice,
                stock, minStock, location, taxCode
            } = req.body;

            if (!barcode || !name || !salePrice || !purchasePrice) {
                res.status(400).json({ 
                    error: 'Faltan campos requeridos: barcode, name, salePrice, purchasePrice' 
                });
                return;
            }

            const product = await this.productService.createProduct(
                barcode, name, salePrice, purchasePrice, userId,
                categoryId, description, offerPrice, stock, minStock, location, taxCode
            );
    ...
    update = async (req: Request, res: Response): Promise<void> => {
        try {
            const  id  = req.params.id as string;
            const { name, description, categoryId, purchasePrice, salePrice, offerPrice, minStock, location, taxCode, isActive } = req.body;

            const product = await this.productService.updateProduct(id, {
                name, description, categoryId, purchasePrice, salePrice, offerPrice, minStock, location, taxCode, isActive
            });
