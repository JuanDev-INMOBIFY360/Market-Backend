...
export class EmployeeController {
    ...
    create = async (req: Request, res: Response): Promise<void> => {
        try {
            const {
                code, fullName, documentNumber, role, password,
                documentType, phone, email, hireDate
            } = req.body;

            if (!code || !fullName || !documentNumber || !role || !password) {
                res.status(400).json({ error: 'Faltan campos requeridos: code, fullName, documentNumber, role, password' });
                return;
            }

            const employee = await this.employeeService.createEmployee(
                code, fullName, documentNumber, role, password,
                documentType, phone, email, hireDate ? new Date(hireDate) : undefined
            );
   ...
   getById = async (req: Request, res: Response): Promise<void> => {
    ...
    login = async (req: Request, res:Response) : Promise <void> =>{
        try {
            ...
            const {employee, token} = await this.employeeService.login(code, password)
            res.status(200).json({
