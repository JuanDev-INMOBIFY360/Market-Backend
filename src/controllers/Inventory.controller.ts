    ...
    adjustStock = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            if (!productId || newStock === undefined) {
                res.status(400).json({ error: 'Producto y nuevo stock son requeridos' });
                return;
            }
            ...
            const movement = await this.inventoryService.adjustStock(productId, newStock, employeeId, reason);
            res.status(200).json({
