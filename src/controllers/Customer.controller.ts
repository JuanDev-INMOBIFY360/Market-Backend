...
export class CustomerController {
    ...
    create = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            const { documentNumber, fullName, documentType, phone, email, address } = req.body;
            ...
            const customer = await this.customerService.save(
                documentNumber, fullName, documentType, phone, email, address
            );
            res.status(201).json({
    ...
    addPoints = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!points || points <= 0) {
                res.status(400).json({ error: 'La cantidad de puntos debe ser positiva' });
                return;
            }
            if (!reason) {
            ...
            const customer = await this.customerService.addPointsManually(id, points, reason);
            res.status(200).json({
