...
export class ReportService {
	...
	async getEmployeePerformance(startDate?: Date, endDate?: Date): Promise<any> {
		...
		const totalTransactions = ranking.reduce(
			(sum, e) => sum + e.transactions,
			0,
		);
	...
	async getTaxReport(startDate?: Date, endDate?: Date): Promise<any> {
		...
		const totalTaxCollected = sales.reduce(
			(sum, sale) => sum + (Number(sale.tax) || 0),
			0,
		);
		const totalTaxPaid = purchases.reduce(
			(sum: number, purchase: any) => sum + (purchase.tax || 0),
			0,
		);
