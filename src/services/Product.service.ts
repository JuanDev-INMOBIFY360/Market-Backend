...
export class ProductService {
    ...
    async createProduct(
        ...
        if (existingBarcode) {
            throw new Error(`Ya existe un producto con el código de barras ${barcode}`);
        }
    ...
    async getAllProducts(page: number = 1, limit: number = 50): Promise<{ products: any[]; total: number }> {
    ...
    async toggleProduct(id: string): Promise<Products> {
        const product = await this.getProductById(id);
        const updated = await this.productRepository.update(id, { isActive: !product.isActive });
        if (!updated) {
