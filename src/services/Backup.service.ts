...
export class BackupService {
    ...
    // ============ LISTAR BACKUPS ============
    async listBackups(): Promise<{ filename: string; size: number; createdAt: Date }[]> {
        this.ensureBackupDir();
    ...
    async createFullBackup(): Promise<{ filename: string; size: number; createdAt: Date }> {
        ...
        return new Promise((resolve, reject) => {
            ...
            archive.pipe(output);
            archive.file(path.join(this.backupDir, dbBackup.filename), { name: dbBackup.filename });
            archive.finalize();
        });
    }
}