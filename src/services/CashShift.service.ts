...
export class CashShiftService {
    ...
    async openShift(employeeId: string, openingBalance: number): Promise<CashShift> {
        const currentShift = await this.cashShiftRepository.findCurrentShiftByEmployee(employeeId);
        if (currentShift) {
    ...
    async closeShift(employeeId: string, closingBalance: number, notes?: string): Promise<CashShift> {
        const currentShift = await this.cashShiftRepository.findCurrentShiftByEmployee(employeeId);
        if (!currentShift) {
        ...
        const movements = await this.cashMovementRepository.findByShift(currentShift.id);
    ...
    async getCurrentShift(employeeId: string): Promise<CashShift | null> {
        return await this.cashShiftRepository.findCurrentShiftByEmployee(employeeId);
    }
