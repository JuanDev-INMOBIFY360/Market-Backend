...
export class DashboardService {
    ...
    async getTodaySales(): Promise<{ total: number; count: number; average: number }> {
    ...
    //VENTAS DEL MES 
    async getMonthSales(): Promise<{ total: number; count: number; average: number; comparison: number; }> {
        const now = new Date();
    ...
    async getLowStockCount(): Promise<{ count: number; products: { name: string; stock: number; minStock: number }[] }> {
    ...
    async getActiveShifts(): Promise<{ count: number; shifts: { employeeName: string; openedAt: Date; salesCount?: number }[] }> {
    ...
    async getSalesByHour(): Promise<{ hours: string[]; totals: number[]; transactions: number[] }> {
        const now = new Date();
    ...
    async getPaymentMethods(): Promise<{ method: string; count: number; total: number; percentage: number }[]> {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
...
async getTopProductsToday(limit: number = 5): Promise<{ name: string; quantity: number; total: number }[]> {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        ...
        const productStats = new Map<string, { name: string; quantity: number; total: number }>();
