...
export class EmployeeService {
    ...
    async createEmployee(
        ...
        const existingByDoc = await this.employeeRepository.findByDocumentNumber(documentNumber);
        if (existingByDoc) {
            throw new Error(`Ya existe un empleado con el documento ${documentNumber}`);
        }
        ...
        if (role === 'owner') {
            ...
            if (ownerCount > 0) {
                throw new Error('Ya existe un dueño del sistema. Solo puede haber uno.');
            }
        }
    ...
    async login(code: string, password: string): Promise<{ employee: Employee; token: string }> {
